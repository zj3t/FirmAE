...
def connectionCheck():
    try:
        r = requests.get('http://'+TARGET,timeout = 1)
        return True
...
def send_http_dummy(port, uri, method, headers={}, params={}, need_login=False):
    ...
    try:
        if method == 'get':
            ...
            print("[O]send_http_GET_dummy_headers: "+str(r.request.headers))
            print("[O]send_http_GET_dummy_data: "+str(params))    
        elif method == 'post':
...
def get_hnap_reqs(template, idx,payload):
        ...
        for tmp_idx in range(length):
            if cur_idx == tmp_idx:
                if MODE == 'bof':
                    ...
                    data_list[tmp_idx] += PREFIX + string + '>'+payload + str(idx)
            else:
...
def fuzz(infos, cgi_map, hnap_map, ssdp_list, action_map, payload):
    dummy_idx_list = []
    idx = 0
    for info in infos:
        print('[*] Atacck Port: ' + info['port'])
        port = int(info['port'])
        if port in [80, 8181, 8080]:
            for hnap in hnap_map:
                ...
                send_hnap_dummy(port=port, hnap=hnap + payload + str(idx))
                dummy_idx_list.append(['hnap_header', port, hnap])
                '''
                if connectionCheck() == False:
                    print("\n\n\n")
                    print(dummy_idx_list[-1])
                    sys.exit(0)
                '''
                idx += 1

                # send hnap data signature
                data_list = get_hnap_reqs(hnap_map[hnap], idx, payload)
                for data in data_list:
                    ...
                    dummy_idx_list.append(['hnap_data', port, hnap, data.replace(payload + str(idx), '{payload}'), need_login])
                    '''
                    if connectionCheck() == False:
                        print("\n\n\n")
                        print(dummy_idx_list[-1])
            ...
            send_http_dummy(port=port, method='get', uri='/', headers={'Cookie': 'i:'+payload + str(idx)})
            ...
            for uri in cgi_map['uri']:
                for param in cgi_map['param']:
                    if MODE == 'bof':
                        ...
                        send_soap_dummy(port, uri, param, payload + str(idx), '')
                ...
                for header in cgi_map['header']:
                    if MODE == 'bof':
                        ...
                        send_soap_dummy(port, uri, '', '', '{} '+payload+'{}\n'.format(header, idx))
        ...
        if port in [1900, 5000]:
            for ssdp in ssdp_list:
                if MODE == 'bof':
                    ...
                    send_ssdp_dummy(port, ssdp + payload + str(idx))
...
def get_execve_log(dummy_length):
    execve_list = []
    for line in command('strings ../scratch/{}/qemu.final.serial.log | grep "\[ANALYZE\]" | grep -i zj3t'.format(IID)):
...
def main():
    ...
    if infos:
        for payload in PAYLOAD_LIST:
            input("Attack "+str(cnt+1)+": "+str(payload))
            dummy_list = fuzz(infos, cgi_map, hnap_map, ssdp_list, action_map, payload)
